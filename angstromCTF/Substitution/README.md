...
So, if we knew the flag is 4 characters long, we could create 4 equations of this kind by making the algorithm encrypt for example 0,1,2,3. We'd have 4 unknowns and for linear equations moodulo 691 so we can solve for these unknowns. The only problem now is that we don't know the length of the flag, so we have to try many possible lengths, solve the equations for each, convert the a's back to characters and see if the results contain anything sensible.
